...
function assertCalculatorShape(shape) {
  if (shape !== "calculator-expression") {
    throw new PenumbraError("INPUT_SHAPE_INVALID", "Submitted input shape is unsupported.");
  }
}

function isReturnableState(state) {
  return state === "Active" || state === "Decoyed";
}
  ...
  for (const inputEvent of events) {
    switch (inputEvent?.type) {
      ...
      case "submit":
        ...
        assertCalculatorShape(inputEvent.shape);
      ...
      case "lock":
      case "restart":
        if (!isReturnableState(state)) invalidTransition();
